"""
Common Name Checklist Questions Extractor for CFIA Compliance.

This module extracts checklist questions from the CFIA Food Labelling
Requirements Checklist - Common Name section using:
...
Usage:
    python checklist_questions/common_name.py

Output:
    checklist_questions/common_name_questions.json
"""
...
OUTPUT_FILE = OUTPUT_DIR / "common_name.json"


def extract_common_name_questions(
    api_key: Optional[str] = None,
    save_to_file: bool = True
) -> Dict[str, Any]:
    """
    Extract Common Name checklist questions from CFIA website.
    ...
    """
    print("=" * 60)
    print("CFIA Common Name Checklist Questions Extractor")
    print("=" * 60)
    print(f"Source: {CFIA_REQUIREMENTS_CHECKLIST_URL}")
    print(f"Section: Common Name")
    print("-" * 60)
    
    # Step 1: Scrape the raw section text
    print("Step 1: Scraping raw section text...")
    raw_text = scrape_section("Common name", "Net quantity declaration")
    ...
    prompt = f"""
Below is the raw text scraped from the CFIA Food Labelling Requirements Checklist - Common Name section.
...
def get_questions() -> List[Dict[str, Any]]:
    """
    Get Common Name questions - loads from JSON if available, otherwise extracts fresh.
    ...
    result = extract_common_name_questions(save_to_file=True)
...
if __name__ == "__main__":
    ...
    result = extract_common_name_questions()
